...
class JobContext(contextlib.AsyncExitStack):
    ...
    def prepare_secrets(self, names: Sequence[str], tmpdir: Path) -> Sequence[str]:
        def get_args(name: str) -> Sequence[str]:
            try:
                return self._container_secrets[name]
            except KeyError:
                raise LookupError(f'no container.secrets.{name} entry') from None

        @functools.cache  # cached until this function returns
        def get_secret(secret_name: str) -> str:
            if secret_name in self._external_secrets:
                return os.path.expanduser(self._external_secrets[secret_name])
            try:
                tmpdir.mkdir(parents=True, exist_ok=True)
                return str(unpack_inline_secret(tmpdir, self._inline_secrets, secret_name))
            except KeyError:
                raise LookupError(f'secret %{{{secret_name}}} is not configured') from None

        return [
            re.sub(r'%\{(?P<secret>[^}]+)\}', lambda m: get_secret(m['secret']), arg)
            for name in names
            for arg in get_args(name)
        ]
