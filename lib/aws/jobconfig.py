...
def provide_secrets(
    ...
    if "github-token" in secrets:
        yield "github-token", params["secrets/github-token"]

    if "fedora-wiki" in secrets:
        yield "fedora-wiki", params["secrets/fedora-wiki"]

    if "fedora-wiki-staging" in secrets:
        yield "fedora-wiki-staging", params["secrets/fedora-wiki-staging"]
    ...
    *,
    ...
    return {
        ...
        "forge": {
            "github": {
                "post": post,
                "token": params["secrets/github-token"],
            }
        },
        "logs": {
            "attach-journal": True,
