...
### Infrastructure deployment
 ...
 - [`infra_definitions.py`](infra_definitions.py): the definition of our AWS
   ...
   runners.  Config parameters live directly under the path defined by
   `DISPATCHER_PARAMS` in `account.py`; secrets live under a `/secrets/`
   subpath of the same prefix.
...
package can also be very helpful (for the `aws` CLI tool).
...
group](https://rover.redhat.com/groups/group/cockpit-infra-accounts).
...
changes to infrastructure should be made to `infra_definitions.py`.
...
You'll want a checkout of the `ci-secrets` repository, mentioned above.

```
AWS_PROFILE=cockpit-ci-infractl python3 -m lib.aws.infractl dispatcher set secrets \
    --secrets-dir $XDG_RUNTIME_DIR/ci-secrets/aws-secrets/dispatcher/
```

Files from the secrets directory are uploaded as `SecureString` parameters under
the `/cockpit-ci/dispatcher/secrets/` SSM path (one parameter per file, named
after the file).  You'll need to restart the dispatcher afterwards to pick up
the new values.
...
available.  This is good for inspecting job runs and debugging issues.  It's
also good for debugging issues with the runner setup: any local changes made to
the job-runner config or runner ignition config will be picked up and used.
