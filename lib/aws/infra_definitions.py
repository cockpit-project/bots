...
def sync_iam() -> None:
    ...
    # Users
    ensure_user(
        # This is useful for testing the dispatcher code locally or for using
        # lib.aws.launch_runner to spawn testing jobs (also locally).
        "cockpit-ci",
        managed_policies=[
            policy_dispatcher,
...
def update_bots_urls(
    cockpit_bots_url: str, *, dispatcher: bool = True, runner: bool = True
) -> None:
    if dispatcher:
...
def update_max_awaiting_logs(count: int) -> None:
    ensure_parameter(f"{DISPATCHER_PARAMS}/max-awaiting-logs", str(count))


def upload_secrets(secrets: Mapping[str, str]) -> None:
