...
def ensure_policy(name: str, statements: Sequence[JsonObject]) -> str:
    ...
    iam.create_policy_version(
        PolicyArn=arn, PolicyDocument=policy_document(statements), SetAsDefault=True
    )
...
def ensure_role(
    ...
    *,
    ...
    iam.update_role(
        RoleName=name, MaxSessionDuration=int(max_session_duration.total_seconds())
    )
...
def ensure_user(
    ...
    with _suppress("EntityAlreadyExists"):
        iam.create_user(
            UserName=name, Tags=[{"Key": k, "Value": v} for k, v in TAGS.items()]
        )
...
def ensure_security_group(
    ...
    *,
    ...
    ec2.create_tags(
        Resources=[group_id], Tags=[{"Key": k, "Value": v} for k, v in TAGS.items()]
    )
    ...
    if ingress:
        ec2.authorize_security_group_ingress(
            GroupId=group_id, IpPermissions=list(ingress)
        )
    print("    - synced")
...
def ensure_launch_template(
    ...
    *,
    ...
    # before adding ours, then clean up the old default afterwards.
    versions = ec2.describe_launch_template_versions(LaunchTemplateName=name)[
...
def ensure_parameter(
    name: str, value: str, param_type: ParameterTypeType = "String"
) -> None:
    ...
    for reservation in resp["Reservations"]:
        for instance in reservation["Instances"]:
            ...
            for ni in instance.get("NetworkInterfaces", ()):
                result[f"{arn_prefix}:network-interface/{ni['NetworkInterfaceId']}"] = (
                    name
                )
...
def check_unmanaged() -> set[str]:
    ...
    explorer = boto3.client("resource-explorer-2", region_name='us-east-1')
    resources = {
        r["Arn"]
        for page in explorer.get_paginator("search").paginate(
            QueryString=RESOURCES_QUERY
        )
    ...
    runners = {
        ...
        for arn, name in _collect_instance_arns([
            ...
            if fnmatch.fnmatch(arn, "arn:aws:ec2:*:instance/*")
        ]).items()
