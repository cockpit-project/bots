            ...
            if (m) {
                ...
                if (segment.indexOf("# RETRY") !== -1) {
                    if (segment.indexOf("(test affected tests 3 times)") !== -1) {
                        ...
                        entry.passed = true;
                        entry.retried = true;
                        entry.title = entry.title.replace("RETRY", "<mark>RETRY</mark>");
                    }
                } else if(m[1] == "ok") {
...
function set_content(text) {
    ...
    Array.from(log.getElementsByClassName("test-entry")).forEach(el => {
    ...
    Array.from(log.querySelectorAll("[data-links-row]")).forEach(el => {
