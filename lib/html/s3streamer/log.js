...
function find_patterns(segment) {
    ...
    for (let i = 0; i < link_patterns.length; i++) {
        const p = link_patterns[i];
        if (!p.pattern) {
            continue;
        }
        const r = new RegExp(p.pattern, "gm");
        const currentPattern = [];
        for (const m of segment.matchAll(r)) {
...
function extract(text) {
    ...
    if (tap_range_match) {
        ...
        segments.forEach((segment, segmentIndex) => {
            ...
            const entry = { passed: true, skipped: false, retried: false, links: [], text: segment };
        ...
        entries.sort((a, b) => {
            a = Number.isNaN(parseInt(a.idx, 10)) ? a.idx : parseInt(a.idx, 10);
            b = Number.isNaN(parseInt(b.idx, 10)) ? b.idx : parseInt(b.idx, 10);
...
class LogViewer extends LitElement {
    ...
    render_parsed(tap) {
        ...
        const sorted_entries = [...tap.entries].sort((a, b) => {
            const a_idx = Number.isNaN(parseInt(a.idx, 10)) ? a.idx : parseInt(a.idx, 10);
            const b_idx = Number.isNaN(parseInt(b.idx, 10)) ? b.idx : parseInt(b.idx, 10);
