...
let link_patterns = [
    {
        ...
        "pattern": "Pixel test ([A-Za-z0-9_\\-\\.]+) failed:",
...
function find_patterns(segment) {
    function fmt(tmpl, match) {
        return tmpl.replace(/\$([0-9]+)/g, (m, x) => match[Number(x)]);
    }
    ...
    for (let i = 0; i < link_patterns.length; i++) {
        ...
        while (m = r.exec(segment)) {
            ...
            currentPattern.push({ url, title, icon, label, color });
        }
        if (currentPattern.length) {
            links.push(currentPattern);
        }
    }
    return links;
}
...
function extract(text) {
    ...
    if (tap_range_match) {
        ...
        return { title: text.slice(0, text.indexOf('\n')), entries, total, passed, failed, skipped, retries, affected_retries, total_test_time };
    }
...
class LogViewer extends LitElement {
            ...
            <div>
                <label class="pf-v6-c-switch">
                    <input class="pf-v6-c-switch__input" type="checkbox" role="switch"
                           ...
                           @change=${e => { this.show_only_failed = e.target.checked; }}>
    ...
    render_parsed(tap) {
        ...
        const sorted_entries = [...tap.entries].sort((a, b) => {
            const a_idx = isNaN(parseInt(a.idx, 10)) ? a.idx : parseInt(a.idx, 10);
            const b_idx = isNaN(parseInt(b.idx, 10)) ? b.idx : parseInt(b.idx, 10);
        ...
        const has_failed = tap.entries.some((e) => e.failed && !e.skipped);
        const visible_entries = (this.show_only_failed && has_failed)
        ...
        return html`
            ...
            <ul class="pf-v6-c-data-list pf-m-compact">
                ${repeat(visible_entries, e => e.id, entry => html`<test-entry .entry=${entry}></test-entry>`)}
            </ul>`;
    }
