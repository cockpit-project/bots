import { html, LitElement, live, nothing, repeat } from "lit";
...
function parse_tap(text) {
    ...
    const tap_result = /^(?<ok>ok|not ok) (?<num>\d+) (?<description>.*)\n/gm;
    ...
    for (const m of body.matchAll(tap_result)) {
        ...
        const skip = m.groups?.description?.match(/[^#]+# SKIP /);
        const todo = m.groups?.description?.match(/[^#]+# TODO /);
        const retry = m.groups?.description?.match(/[^#]+# RETRY (\d+)/);
                ...
                <div class="pf-v6-c-data-list__item-row">
                    ...
                    <div class="pf-v6-c-data-list__item-content">
                        <div class="pf-v6-c-data-list__cell">
                            ...
                            </a>${entry.description ? html`:&nbsp;<span>${entry.description}</span>` : nothing}
                            <div>
            ...
            <div>
                <label class="pf-v6-c-switch">
                    <input class="pf-v6-c-switch__input" type="checkbox" role="switch"
                           .checked=${live(has_failed ? this.show_only_failed : false)}
